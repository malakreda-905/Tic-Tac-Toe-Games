#ifndef MINMAXPLAYER3X3X_H
#define MINMAXPLAYER3X3X_H
...
int X_O_MinMax_3x3X_Player<T>::calculateMinMax(T s, bool isMaximizing, int depth) {
    if (this->boardPtr->is_win()) {
        // In Misere, getting three in a row is bad
        return isMaximizing ? 1 : -1;  // Reversed from normal Tic Tac Toe
    }
...
pair<int, int> X_O_MinMax_3x3X_Player<T>::getBestMove() {
    int bestValue = numeric_limits<int>::max();  // Changed to max for Misere
    pair<int, int> bestMove = {-1, -1};
    T opponentSymbol = (this->symbol == 'X') ? 'O' : 'X';

    //  we want to avoid making three in a row
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            if (this->boardPtr->update_board(i, j, this->symbol)) {
                ...
                //  we want to minimize our chances of making three in a row
